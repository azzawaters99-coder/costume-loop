...
export default function ListingDetail() {
  ...
  // Check which measurements exist
  const measurements = [
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="md:w-1/2">
            ...
            {measurements.length > 0 && (
              <div style={{ background: '#faf7f2', border: '1px solid #e8dcc8', borderRadius: 12, padding: 16, marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
                  ...
                  <p style={{ fontSize: 13, fontWeight: 700, color: '#4a0e2e' }}>Measurements</p>
                  <span style={{ fontSize: 11, color: '#bbb', marginLeft: 2 }}>(cm)</span>
                </div>
                ...
                <p style={{ fontSize: 11, color: '#bbb', marginTop: 10, lineHeight: 1.5 }}>
                  Tip: measure the dancer and add 2-3cm ease for comfort.
                </p>
              </div>
            )}
            ...
            <div className="flex flex-col gap-3">
              {isRental ? (
                <>
                  <button onClick={() => setEnquiryOpen(true)} className="w-full py-3 rounded-xl text-white font-semibold text-sm" style={{ backgroundColor: '#2e7d32', fontSize: 15 }}>
                    🔄 Enquire to Rent
                  </button>
                  <button className="w-full py-3 rounded-xl font-semibold text-sm border" style={{ borderColor: '#800020', color: '#800020', backgroundColor: 'transparent' }} onClick={() => setEnquiryOpen(true)}>
                    Message Seller
                  </button>
                </>
              ) : (
                <>
                  <button className="w-full py-3 rounded-xl text-white font-semibold text-sm" style={{ backgroundColor: '#c49a2a' }}>Message Seller</button>
                  <button className="w-full py-3 rounded-xl font-semibold text-sm border" style={{ borderColor: '#800020', color: '#800020', backgroundColor: 'transparent' }}>
          ...
          <div style={{ background: 'white', borderRadius: 20, padding: 32, width: '100%', maxWidth: 460, position: 'relative' }}>
            ...
            <form onSubmit={handleEnquirySubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <input required value={enquiryName} onChange={e => setEnquiryName(e.target.value)} placeholder="Jane Smith"
              ...
              <div>
                ...
                <input required type="email" value={enquiryEmail} onChange={e => setEnquiryEmail(e.target.value)} placeholder="jane@example.com"
                  style={{ width: '100%', border: '1px solid #e5e5e5', borderRadius: 10, padding: '10px 14px', fontSize: 14, outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#4a0e2e' }}>Message</label>
                <textarea required value={enquiryMessage} onChange={e => setEnquiryMessage(e.target.value)} rows={4}
              ...
              <button type="submit" style={{ width: '100%', padding: '13px', borderRadius: 12, border: 'none', background: '#2e7d32', color: 'white', fontWeight: 700, fontSize: 15, cursor: 'pointer', marginTop: 4 }}>
                Send Enquiry
              </button>
            </form>
