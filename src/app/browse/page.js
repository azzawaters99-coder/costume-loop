...
export default function BrowsePage() {
    ...
    const filtered = listings.filter(l => {
          ...
          const matchSearch = !search || l.title?.toLowerCase().includes(q) || l.genre?.toLowerCase().includes(q) || l.location?.toLowerCase().includes(q) || l.description?.toLowerCase().includes(q);
    ...
    const resetFilters = () => { setSelectedGenres([]); setSelectedConditions([]); setSelectedTypes([]); setMaxPrice(500); setSearch(''); };
...
{/* Listings grid */}
          ...
          {loading ? (
                    ...
                    <div className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow cursor-pointer" style={{ height: '100%', border: listing.featured ? '2px solid #c49a2a' : 'none' }}>
...
{listing.images && listing.images.length > 0 ? (
                            <img src={listing.images[0]} alt={listing.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                                      ...
                                                      <span className="text-5xl">ð</span>
                                                                            ...
                                                                            <div className="p-4">
                                                    {listing.featured && <span style={{background:'#c49a2a',color:'white',fontSize:11,fontWeight:700,padding:'2px 8px',borderRadius:4,marginBottom:4,display:'inline-block'}}>â­ Featured</span>}
                                                                              ...
                                                                              <p className="text-xs text-gray-500 mb-1">{listing.genre} Â· {listing.size}</p>
                                                                              <p className="text-xs text-gray-400">ð {listing.location}</p>
...
}onst genres = ['Ballet', 'Contemporary', 'Jazz & Tap', 'Cultural & Character', 'Acrobatics', 'Hip Hop', 'Musical Theatre'];
const conditions = ['New with tags', 'Like new', 'Good', 'Fair'];
const listingTypes = ['Sell', 'Swap', 'Hire'];
